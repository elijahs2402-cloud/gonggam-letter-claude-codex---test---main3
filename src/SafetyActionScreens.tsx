...
function ReplyReportForm({ letterId, complete }: { letterId?: string; complete: boolean }) {
  ...
  if (!letter || !reply || letter.senderId !== userId) return <Shell title="편지 신고" fallback="/mailbox"><section className="flow-message"><h1>신고할 편지를 찾을 수 없어요</h1><button className="flow-primary-button" type="button" onClick={() => navigateTo("/mailbox")}>편지함 가기</button></section></Shell>;
  ...
  if (status === "complete") return <Shell title="신고 접수" fallback={returnTo} showBackButton={false}><section className="flow-message"><h1>{withBlock ? "신고를 접수하고 작성자를 차단했어요" : "신고를 접수했어요"}</h1><p>신고 내역은 차단 및 신고 관리에서 확인할 수 있어요.</p><button className="flow-primary-button" type="button" onClick={() => navigateTo("/safety-management")}>신고 내역 확인</button><button className="flow-text-button" type="button" onClick={() => navigateTo("/home")}>홈으로 돌아가기</button></section></Shell>;
...
export function LetterReturnScreen({ letterId }: { letterId?: string }) {
  ...
  if (phase === "complete") return <Shell title="편지 두고 가기" fallback="/home" showBackButton={false}><section className="flow-message"><h1>편지를 두고 왔어요</h1><p>이 편지는 다시 누군가를 기다려요.</p><button className="flow-primary-button" type="button" onClick={() => navigateTo(getListenEntryPath(getCurrentUserId()))}>다른 편지 만나기</button><button className="flow-text-button" type="button" onClick={() => navigateTo("/home")}>홈으로 돌아가기</button></section></Shell>;
  // 완료 직전의 사이 화면.
  ...
  if (!letter || letter.assignedReaderId !== readerId || !["assigned", "read", "waiting_for_reply"].includes(letter.status) || getLetterReturn(letter.id, readerId)) return <Shell title="편지 두고 가기" fallback="/home" showBackButton={false}><section className="flow-message"><h1>{RETURNED_LETTER_TITLE}</h1><p>{RETURNED_LETTER_BODY}</p><button className="flow-primary-button" type="button" onClick={() => navigateTo(getListenEntryPath(getCurrentUserId()))}>다른 편지 만나기</button><button className="flow-text-button" type="button" onClick={() => navigateTo("/home")}>홈으로 돌아가기</button></section></Shell>;
