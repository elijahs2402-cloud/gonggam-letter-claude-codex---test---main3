...
export function LetterReportScreen({ letterId }: { letterId?: string }) {
    ...
    return (
      <Shell title="신고 접수">
        <section className="flow-message">
          ...
          <small className="flow-state-helper">
            신고 접수만으로 운영 결과가 바로 결정되지는 않아요.
          </small>
          <button
            className="flow-primary-button"
            type="button"
  ...
  return (
    <Shell title="편지 신고">
      <section className="report-screen report-screen--expanded">
        <h1>이 편지를 신고할까요?</h1>
        <p>다른 사람을 해치거나 불편하게 만드는 내용이 있다면 알려주세요.</p>
        <fieldset>
          <legend>신고 사유</legend>
        ...
        <label className="report-detail-label">
          추가 설명 <span>선택</span>
          <textarea
            value={detail}
            onChange={(event) => setDetail(event.target.value)}
            placeholder="개인정보는 적지 않아도 괜찮아요."
          />
        </label>
        <label className="report-block-choice">
          ...
          />
          <span>
            <strong>이 편지를 대기 목록에서 숨기기</strong>
            <small>신고와 별개로 내 목록에서만 숨겨져요.</small>
          </span>
        </label>
        <label className="report-block-choice">
          <input
          ...
          />
          <span>
            <strong>이 작성자도 함께 차단하기</strong>
            <small>차단하면 앞으로 이 사용자의 편지가 나타나지 않아요.</small>
          </span>
        </label>
