...
│   │   ├── TermsOfService/ · ServiceNotice/                             # 서비스 이용약관 · 서비스 안내 세부(숨은 화면)
...
- 홈 하단 소식 카드는 1차 오픈에서 빠졌다. 화면 코드는 `pages/Home/Home.tsx` 에 남아 있고, `getHomeNotice()` 가 항상 `null` 을 돌려주게 막아 두었다. 실제 데이터는 연결되지 않았다.
